...
sudo docker run -it --rm -v $(pwd)/my_test_data:/data -v $(pwd)/my_test_config:/config -p 5000:80 --device=/dev/video0:/dev/video0 --entrypoint /bin/sh local/octoprint_test
